...
final class ItemPodcastingHandler: TagHandler {
	...
	func processTag(_ tagName: String, text: String, withAttributes attributesDict: [String : String]) throws(SyndicationFeedError) {
		guard Podcasting.itemTags.contains(tagName) else {
		...
		switch tagName {
			...
			case Podcasting.Season.tagName:
				...
				let season = try mapper.mapToSeason(from: attributesDict, seasonValue: text)
				details.season = season
			case Podcasting.Episode.tagName:
				let mapper = EpisodeMapper()
				let itemEpisode = try mapper.mapToEpisode(from: attributesDict, episodeValue: text)
				details.episode = itemEpisode
			case Podcasting.Chapters.tagName:
				let mapper = ChaptersMapper()
				let chapters = try? mapper.mapToChapter(from: attributesDict)
